...
def train_model():
    ...
    for i in range(len(movies)):
        vector = tfidf_matrix[i].toarray().flatten().astype("float32")
        annoy_index.add_item(i, vector)
    #деревья для индекса
    annoy_index.build(5)
    #сохраняем для использования позже
    joblib.dump((tfidf, tfidf_matrix), "models/tfidf_annoy.pkl")
    annoy_index.save("models/tfidf_annoy_index.ann")

def get_similar_movies(movie_id: int, top_n: int = 10):
    tfidf, tfidf_matrix = joblib.load("models/tfidf_annoy.pkl")
    vector_dim = tfidf_matrix.shape[1]
    annoy_index = AnnoyIndex(vector_dim, metric="angular")
    annoy_index.load("models/tfidf_annoy_index.ann")
